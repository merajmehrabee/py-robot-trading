...
from td.utls import milliseconds_since_epoch
...
from pyrobot.portfolio import protfolio
...
class PyRobat():
    ...
    def _init_(self,client_id:str,redirect_uri:str,credentials_path: str =None, trading_account:str=None, paper_trading: bool = True)->None:
...
def pre_market_open(self) -> bool:
    ...
    pre_market_start_time = datetime.utcnow().replace(hour=12,minte=00, second=00).timestamp()
    market_start_time = datetime.utcnow().replace(hour=12,minte=00, second=00).timestamp()
...
def post_market_open(self) -> bool:
       ...
       post_market_end_time = datetime.utcnow().replace(hour=22,minte=30, second=00).timestamp()
       market_end_time = datetime.utcnow().replace(hour=22,minte=30, second=00).timestamp()
       right_now = datetime.utcnow().timestamp()
...
def regular_market_open(self) -> bool:
    ...
    market_start_time = datetime.utcnow().replace(hour=13,minte=30, second=00, tzinfo=timezone.utc).timestamp()
    market_end_time = datetime.utcnow().replace(hour=20,minte=00, second=00, tzinfo=timezone.utc).timestamp()
...
def create_portfolio(self):
    ...
    # Initilize a new Portfolio object.
    self.portifolio = protfolio(account_number=self.trading_account)
...
def grab_current_quotes(self) ->dict:
    ...
    symbol = self.portfilio.positions.keys()

    # # Grab the qutes.
    quotes = self.session.get_quotes(instruments=list(symbol))

    return quotes

def grab_historical_prices(self, start: datetime, end: datetime, bar_size: int = 1, bar_type: str = 'minute', symbol: list[str] = None) -> list(dict):
    ...
    if not symbol:
        symbol = self.protfolio.positions
    ...
    for symbol in symbol:
        ...
        self.historical_price[symbol] = {}
        self.historical_price[symbol]['candles'] = historical_price_response['candles']

        for candel in historical_price_response['candles']:

            new_price_mini_dict = {}
            new_price_mini_dict['symbol'] = symbol
            new_price_mini_dict['open'] = candel['open']
            new_price_mini_dict['close'] = candel['close']
            new_price_mini_dict['high'] = candel['high']
            new_price_mini_dict['low'] = candel['low']
            new_price_mini_dict['volume'] = candel['low']
            new_price_mini_dict['datetime'] = candel['datetime']
            new_price.append(new_price_mini_dict)

    self.historical_prices['aggregated'] = new_price
...
def get_latest_bar(self) -> list[dict]:
    ...
    for symbol in self.portfolio.positions:
        ...
        for candel in historical_price_response['candles'][-1:]:
            ...
            new_price_mini_dict['volume'] = candel['low']
            new_price_mini_dict['datetime'] = candel['datetime']
            latest_price.append(new_price_mini_dict)
