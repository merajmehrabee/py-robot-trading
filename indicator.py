...
class Indicators():
    ...
    def __init__(self, price_data_farme: StockFrame) -> None:
            
            self._stock_frame: StockFrame = price_data_farme
            self._price_groups = self._stock_frame.symbol_groups
            self._current_indicators = {}
            self._indicator_signals = {}
            self._frame = self._stock_frame.farme

    def set_indicator_signal(self, indicator: str, buy: float, sell: float, condition_buy: Any, condition_sell: Any) -> None:

        # if there is no signal for that indicator set tamplate.
        if indicator not in self._indicator_signals:
             self._indicator_signals[indicator] = {}

        # modify the signal
        self._indicator_signals[indicator]['bye'] = buy
        self._indicator_signals[indicator]['sell'] = sell
        self._indicator_signals[indicator]['bye_operator'] = condition_buy
    ...
    def price_data_frame(self, price_data_farme: pd.DataFrame) ->None:
         
         self._frame = price_data_farme
    ...
    def rsi(self, period: int, method: str = 'wilders') -> pd.DataFarme:
         ...
         self._frame.drop(
              labels=['ewma-up', 'ewma_down', 'down_day', 'up_day', 'change_in_price'],
              axis=1,
              inplace=True
         )
    ...
    def refresh(self):
         ...
         # Loop through all the stared indicators
